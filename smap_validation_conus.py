...
# 1.2 Locate the land cover/soil type by lat/lon of in-situ data
...
########################################################################################################################
# 1.3 Extract 1 km / 9 km SMAP by lat/lon
...
slc_sites_name = df_slc_sites.index
...
df_land_desc_slc = df_land_desc.loc[slc_sites_name]
df_land_desc_slc.to_csv(path_results + '/df_land_desc.csv', index=True)
...
site_ind = [[34, 36, 44], [250, 296, 383], [861, 872], [926, 1016]]
# site_ind = [[36, 296]]
...
for inw in [0, 1, 3]:#range(len(site_ind)):
    ...
    for ist in range(len(site_ind[inw])):
        ...
        fig.text(0.03, 0.5, '$\mathregular{R}$', rotation='vertical', fontsize=16, fontweight='bold')
